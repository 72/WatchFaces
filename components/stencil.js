...
import * as digit from './stencilNumbers.js';

// Setup UI elements
let app = document.querySelector('#App');
let container = document.querySelector('#watchFaceContainer');
let stencilWrapper = document.querySelector('#stencil svg');
...
function setAspectRatio() {
	...
	if(windowWidth >= windowHeight){
		...
		stencilWrapper.style.height = `${windowHeight + 36}px`;
		stencilWrapper.style.width = 'auto';
	} else {
		baseValue = windowWidth;
		stencilWrapper.style.height = 'auto';
		stencilWrapper.style.width = `${windowWidth * 0.8}px`;
	}
